...
import{defaultPayments,defaultTransactions,loadData,money,Payment,saveData,syncPaymentTransactions,Transaction}from'@/lib/store';
export default function Tagihan(){
 const[p,setP]=useState<Payment[]>(defaultPayments),[processing,setProcessing]=useState(false),[show,setShow]=useState(false),[sel,setSel]=useState(''),[method,setMethod]=useState('transfer'),[msg,setMsg]=useState('');
 useEffect(()=>{const payments=loadData('payments',defaultPayments);syncPaymentTransactions();setP(payments);const q=new URLSearchParams(location.search);const paymentId=q.get('id');if(paymentId){setSel(paymentId);setShow(true);if(q.get('baru')==='1')setMsg('Penghuni baru berhasil dibuat. Tagihan pertama sudah dibuat. Pilih metode pembayaran lalu klik “Simpan & Lunas + Buat Kwitansi”.')}else if(q.get('aksi')==='catat')setShow(true)},[]);
 const pay=()=>{
