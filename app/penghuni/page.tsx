...
export default function Penghuni(){
 const[t,setT]=useState<Tenant[]>(defaultTenants),[r,setR]=useState<Room[]>(defaultRooms),[show,setShow]=useState(false),[name,setName]=useState(''),[room,setRoom]=useState(''),[phone,setPhone]=useState(''),[date,setDate]=useState('2026-09-24'),[rent,setRent]=useState(''),[msg,setMsg]=useState('');
 ...
 const add=()=>{
  ...
  const np=[...loadData('payments',defaultPayments),{id:'P-'+Date.now(),tenant:name,room,month,amount:monthlyRent,status:'unpaid' as const} as Payment];
  setT(tt);setR(nr);saveData('tenants',tt);saveData('rooms',nr);saveData('payments',np);
  setShow(false);setMsg('Penghuni berhasil ditambahkan. Tagihan sewa pertama otomatis dibuat dan siap dilunasi.');
 };
